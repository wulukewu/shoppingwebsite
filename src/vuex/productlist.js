...
export default{
      ...
      try {
        ...
        if (!userId) {
          ...
          const response = await fetch(
              `https://web-cli-14b8e-default-rtdb.asia-southeast1.firebasedatabase.app/orderlist/${userId}.json`,
              {
                  method: 'POST',
                  headers: {
  ...
  getters:{
    ...
    carttotal(state){
      return state.cartlist.reduce((acc, item) => acc + item.price * item.qty, 0);
    },
    cartqty(state){
    ...
    //   try {
    //     const response = await axios.get('https://web-cli-14b8e-default-rtdb.asia-southeast1.firebasedatabase.app/productlist.json');
    //     console.log(response.data);
    //     commit('SET_PRODUCTS', response.data.productlist);
    //   } catch (error) {
...
//   getters: {
//     allProducts(state) {
//         return Array.isArray(state.productlist) ? state.productlist : [state.productlist];
//     },
//     isLoading(state) {
